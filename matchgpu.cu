...
__global__ void gaSelect_depth0(int *match, int *dkeepMatching, int *S_d_ptr, const int nrVertices, const uint random)
{
	//Determine blue and red groups using MD5 hashing.
	//Based on the Wikipedia MD5 hashing pseudocode (http://en.wikipedia.org/wiki/MD5).
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
	...
	//Can this vertex still be matched?
	if (match[i] >= 3 || S_d_ptr[i]!=0) return;
...
__global__ void gMatchEdgeList(int * rmatch, int * cmatch,
...
__global__ void gTentativelyKill(int *match, int *requests, const int nrVertices)
{
...
__global__ void gRestoreLife(int * rmatch, int * cmatch,
								int *match, const int *requests, const int nrVertices)
{
	//Let all blue vertices make requests.
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
...
//==== Random greedy matching kernels ====
__global__ void grRequestEdgeList(int * rmatch, int * cmatch, int *requests, const int *match, int *c_ptr,const int nrVertices)
{
...
__global__ void grRespondEdgeList(int * rmatch, int * cmatch, int *requests, const int *match, const int nrVertices)
{
	//Let all blue vertices make requests.
	const int i = blockIdx.x*blockDim.x + threadIdx.x;
	...
	if (cmatch[i] >= 0){
		...
		int c = rmatch[r];
		//Look at all blue vertices and let them make requests.
		if (match[r] == 0 && match[c] == 1 && requests[r]==c)
		{
			requests[c] = r;
		} else if (match[c] == 0 && match[r] == 1 && requests[c]==r){
			requests[r] = c;
		}
	}
	if (rmatch[i] >= 0){
